...
- ~~**`.q-paneltab` is now dual-consumer** (report panel tabs + the update_language switch) while its rules still live in reportView.css (eagerly bundled via the shell import). Promote to primitives.css in P19/P20.~~ **Done in UIX-4** (2026-07-25): `.q-paneltabs`/`.q-paneltab*`/`.q-panel`/`.q-panel-note` now live in `styles/primitives.css`, and the behaviour moved to `components/panelTabs.ts` (`createPanelTabs`). The language switch needed no change — it keeps borrowing the look and stays `aria-pressed`, deliberately not a tablist.

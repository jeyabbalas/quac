...
| V19 | DuckDB 1.5.x `TRY_CAST` **rounds** decimal strings to integers (`'42.5'`→43, `'0.1'`→0) instead of returning NULL, so `json-schema-subsystem.md §C.1`'s integer ladder (leading `COALESCE(TRY_CAST(raw AS BIGINT), …)`) silently corrupts non-integral values and never produces the `'42.5'` non-integral cast flag. Shipped ladder gates on DOUBLE-integrality first: `CASE WHEN TRY_CAST(s AS DOUBLE) IS NULL THEN NULL WHEN NOT isfinite(…) THEN NULL WHEN … != trunc(…) THEN NULL ELSE COALESCE(TRY_CAST(s AS BIGINT), TRY_CAST(TRY_CAST(s AS DOUBLE) AS BIGINT)) END` (BIGINT branch first keeps exact int64 past 2^53; inner TRY_CAST — plain CAST throws at 1e20 scale). Pinned on BOTH engines: `@duckdb/node-api` 1.5.5 and duckdb-wasm 1.5.4. (V-number may collide with the parallel P11 branch at merge, as in the P02/P03 merge) | ✅ confirmed | P09 `casting.test.ts` + `validation-worker.browser.test.ts` V19 pins, 2026-07-23 |

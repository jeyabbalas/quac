...
| V19 | DuckDB 1.5.x `TRY_CAST` **rounds** decimal strings to integers (`'42.5'`→43, `'0.1'`→0) instead of returning NULL, so `json-schema-subsystem.md §C.1`'s integer ladder (leading `COALESCE(TRY_CAST(raw AS BIGINT), …)`) silently corrupts non-integral values and never produces the `'42.5'` non-integral cast flag. Shipped ladder gates on DOUBLE-integrality first: `CASE WHEN TRY_CAST(s AS DOUBLE) IS NULL THEN NULL WHEN NOT isfinite(…) THEN NULL WHEN … != trunc(…) THEN NULL ELSE COALESCE(TRY_CAST(s AS BIGINT), TRY_CAST(TRY_CAST(s AS DOUBLE) AS BIGINT)) END` (BIGINT branch first keeps exact int64 past 2^53; inner TRY_CAST — plain CAST throws at 1e20 scale). Pinned on BOTH engines: `@duckdb/node-api` 1.5.5 and duckdb-wasm 1.5.4. | ✅ confirmed | P09 `casting.test.ts` + `validation-worker.browser.test.ts` V19 pins, 2026-07-23 |
| V20 | The V17/V18 delimited-text ingest path has a scale ceiling: the `json_extract_string` CTAS re-parses each row's wrapped JSON once per column, and at ~2,000 rows × 266 cols duckdb-wasm dies with `Out of Memory Error: failed to allocate data of size 16.0 MiB (3.1 GiB/3.1 GiB used)` (~2 s in). 101×266 and narrow-wide combinations well below rows×cols×rowJsonBytes ≈ 10⁹ are fine. JSON/parquet ingest of the identical data works (typed `loadData`, no per-cell extraction) — P14's cancel e2e replicates the dirty dataset ×20 via JSON for this reason. P20's 100k×20 perf gate sits near the boundary; revisit the CSV path (chunked CTAS or per-column extraction batches) before raising ingest limits | ✅ confirmed | P14 e2e spike (hesp_big.csv 2,020×266 vs hesp_big.json same data), 2026-07-23 |
| V21 | exceljs 4.4.0 has **no browser streaming writer** — `stream.xlsx.WorkbookWriter` is Node-`fs`-only, so `qc-report-spec.md §5`'s "streamed in 10k-row chunks" applies to READING rows out of DuckDB, not to writing the workbook: the sheet is assembled in memory and emitted with `workbook.xlsx.writeBuffer()` → `Blob`. The library ships as UMD whose API is reachable only under the ESM-interop `default` (`import('exceljs')` → namespace keys `['default','module.exports']`; `mod.Workbook` is undefined, `mod.default.Workbook` is the class) — the writer destructures `{ default: ExcelJS }`. Its guarded browserify `process`/`global`/`self` shims mean no Node polyfills are needed in Vite. Chunk stays lazy (249.9 KB gz), never in the entry graph (bundle gate asserts) | ✅ confirmed | P15 `excelWriter.ts` + `excelRoundtrip.test.ts` + `download.spec.ts`, 2026-07-24 |
| V22 | `@jeyabbalas/data-table` 0.5.1 is a **keyboard trap** (WCAG 2.1.2, Level A): once focus reaches `.dt-root`, neither Tab nor Shift+Tab moves it again — `document.querySelector('.dt-root :focus')` stays null through 900+ presses — while ~1600 focusable controls (266 columns × header buttons) sit inside. **axe does not detect this**; only a keyboard walk does, so `a11y.spec.ts` passing is not evidence against it. QuaC mitigates, and cannot cure: a `.q-skiplink` before the report grid (a `<button>` — an `<a href="#…">` would rewrite the routing hash) plus Escape-to-leave on both grid hosts. Separately, `createDataTable` defaults `colorScheme:'auto'`, whose stylesheet flips the grid dark under `prefers-color-scheme: dark` unless the instance carries `data-dt-color-scheme="light"` — only the option sets it, so it must be passed at every call site. The library's other axe findings are listed in `ui-design.md §9`; re-check them on any data-table bump | ✅ confirmed | P19 keyboard walk + `a11y.spec.ts` diagnostic pass + dark-OS `emulateMedia` check, 2026-07-25 |
| V23 | duckdb-wasm's binder does **NOT implicitly cast VARCHAR in comparisons or arithmetic**: on a schema-less CSV run (all-VARCHAR `quac_typed`), `age BETWEEN 0 AND 120` fails to bind ("Cannot compare values of type VARCHAR and type INTEGER_LITERAL") and `score > age * 10` fails function resolution ("No function matches `*(VARCHAR, INTEGER_LITERAL)`"). Practical consequence for rules-only sessions: such rules never reach the broken-rule RUNTIME policy — lint stage 4's EXPLAIN dry-run (same untyped view) catches them pre-run as row-level `sql-error`s, partial acceptance excludes them (Warning badge), and the remainder runs. String-op and assertion rules without numeric coercion (`unique`, `match_regex`, `no_nulls`, `UPPER`) are typing-immune. Cast-defensive SQL (`TRY_CAST(col AS DOUBLE)`) is the documented author-side fix, and since UIX-16 lint stage 4 **says so in the card** rather than printing the binder text — the exclusion is unchanged, the account of it is not (the P20 README limitation still stands; a README cannot reach someone reading `'+(VARCHAR, VARCHAR)'` inside the slot) | ✅ confirmed | UIX-6 `partialRun.spec.ts` rules-only journey (tiny/people fixtures: R003/R005 excluded, 4/6 executable, run completes), 2026-07-26; UIX-16 measured the HESP example live — clearing the schema excludes **12 of 22** rules across exactly three binder phrase families (3 `No function matches`, 4 `Cannot compare values of type`, 5 `Cannot mix values of type` — 4 BETWEEN + 1 CASE) and `Rules run` falls 20 → 9, 2026-07-27 |

| V24 | `@duckdb/node-api` 1.5.5 result shapes and value types, as the WorkerBridge facade must present them: `getRowObjectsJS()` still returns JS `bigint` for BIGINT columns (the `JS` union includes it by design), so the adapter normalizes **recursively** — LIST and STRUCT payloads carry bigints too, while `Date` and `Uint8Array` are values and pass through. V13's shapes hold unchanged on this engine: DDL → `[]`, DML → one row with a numeric `Count`. Node's `getRowObjectsJS()` (used here) differs from the `getRowObjects()` in the older test adapters (`tests/unit/schema/duckdb.ts`), which returns DuckDB value wrappers for complex types — the JS variant is what gives data-table's `convertBigInts` parity | ✅ confirmed | P20 `nodeBridge.test.ts`, 2026-07-30 |
| V25 | Single-file scans give `__rowid__` = physical file order on node-api, which is the `data-table-api.md` §3 contract ingest depends on (`__rowid__` → `__row__` at CTAS time, and V7's display alignment downstream): `read_json(file)` with `preserve_insertion_order` at its default `true` makes `row_number() OVER () - 1` file order, and `read_parquet(file, file_row_number=true)` supplies it exactly. Pinned with deliberately unsorted payloads (`c, a, b`), so no column ordering could produce the result by accident. node-api has **no buffer registration**, so `loadData` writes bytes to a private `mkdtemp` file and deletes it in a `finally`; `columns` includes `__rowid__` exactly as the wasm loaders report it | ✅ confirmed | P20 `nodeBridge.test.ts`, 2026-07-30 |
| V26 | **`SET enable_external_access = false` IS usable on `@duckdb/node-api` 1.5.5** — the Node counterpart to V6's wasm verdict, and the opposite outcome. What made it unusable in wasm (it kills the COPY export and the registered-buffer `loadData` the display round trip needs) does not arise headless: every `loadData` temp-file read happens BEFORE prepare, and everything after prepare is table-only SQL (corrections CTAS, JS staging, validation SELECTs, report paging). Post-harden, `read_json('/etc/hosts')` is refused while a `CREATE OR REPLACE TABLE … SELECT` still runs. Same one-way semantics as wasm, so a second harden against one instance is wrapped in try/catch. json/parquet/icu/core_functions are **statically linked** in node-api (unlike V11's wasm), so `nodeHarden` issues no `LOAD`s. Also pinned: V23's binder strictness is engine-independent — dropping the headless typed-sync mirror excludes **12 of the 22** HESP rules on node-api, the same 12 UIX-16 measured on duckdb-wasm | ✅ confirmed | P20 `nodeBridge.test.ts` + `nodePipeline.test.ts` (mirror removed → 12 lint errors), 2026-07-30 |
| V27 | **A Vite SSR build copies `publicDir` into `outDir` like any other build** — `headless.md §9`'s recipe ("SSR entry build, deps external, node22") is necessary but not sufficient, because `publicDir` defaults to `public/` and QuaC's `public/duckdb/` holds ~100 MB of self-hosted duckdb-wasm that the native-binding CLI never touches. Every `build:cli` was copying it and `npm pack` shipped it: **23 MB packed / 100 MB unpacked over 43 entries**, for a program whose own code is 300 KB. `publicDir: false` in `vite.cli.config.ts` takes it to **94 KB / 302 KB over 8 entries**. Found by asserting the `npm pack` file list, not by reading the output — the build log reports only the chunks it rendered, so the copied tree is invisible there. Any future second build config off this repo needs the same line | ✅ confirmed | P21 `vite.cli.config.ts` + `cli/run.test.ts` pack assertion, 2026-07-30 |

| V28 | **`optionalDependencies` is not a "don't install this" mechanism — npm's `omit` default is `[]`, so optional deps install exactly like `dependencies` do.** Verified locally: `npm config get omit` returns empty and `npm config ls -l` shows `omit = []`; only an explicit `--omit=optional` (or `NPM_CONFIG_OMIT`) skips them, and nothing sets that by default. `peerDependencies` auto-install too, since npm 7. This matters because it was the obvious-looking escape for P21's finding — mark the browser packages optional and let CLI installers skip them — and it does not work: `@duckdb/duckdb-wasm` alone is **143 MB installed**, and every `npm i -g quac` would still have downloaded it. The mechanism that does work is the only one there is: move them to `devDependencies` and prove the split, which is what P22 did. `tests/cli/dependencies.test.ts` therefore asserts both fields are ABSENT, so a future "optional" browser dep cannot slip past a set-equality check that only reads `dependencies` | ✅ confirmed | P22 `tests/cli/dependencies.test.ts` + `npm config` / `du -sk` measurement, 2026-07-30 |
| V29 | **`@jeyabbalas/data-table`'s package root imports cleanly under plain Node ESM, with no DOM and no browser** — `node --input-type=module -e "import { quoteIdentifier, VERSION } from '@jeyabbalas/data-table'"` resolves, executes and returns `0.5.1`. The module graph reached by the root export is evaluation-safe outside a browser; only the components that touch `document` fail, and importing does not touch them. CJS is a different story and fails hard (`require()` → `ERR_PACKAGE_PATH_NOT_EXPORTED`; the package has no CJS main), which is not a problem here because the repo is `"type": "module"` throughout. This is what lets `tests/unit/core/sql-identifier.test.ts` — the agreement test proving QuaC's lifted `quoteIdentifier` still matches the package's own, character for character — live in the FAST unit tier (318 cases, 256 ms) instead of the browser tier, which is the difference between a check that runs on every `npm run verify` and one that runs when someone remembers | ✅ confirmed | P22 `tests/unit/core/sql-identifier.test.ts`, 2026-07-30 |

| V30 | **npm's name-similarity check runs on PUT, not on GET — a 404 from `npm view <name>` does NOT mean the name is publishable.** `npm view quac` returned `E404 Not Found` (the P21 §9 check, and again on release day), and `npm publish --dry-run` succeeded, because a dry run does not contact the registry's validation. The real publish failed: `E403 … Package name too similar to existing package cac; try renaming your package to '@jeyabbalas/quac'`. The typo-squatting guard compares against every existing name and is only evaluated when the tarball is actually offered, so **nothing available locally can predict it** — the only test is publishing. Scoping is the documented escape and costs nothing user-facing: `bin` names the COMMAND, so `@jeyabbalas/quac` still installs `quac`. It drags one rule in with it — a scoped package defaults to `restricted`, so `publishConfig: {access: "public"}` is now required (P21's note that "no publishConfig is needed, `quac` is unscoped" died with the name). Both are pinned by `tests/cli/dependencies.test.ts`. Successors: budget for a rename at the moment of first publish, not before it | ✅ confirmed | P22 real `npm publish` attempt, 2026-07-30 |

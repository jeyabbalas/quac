...
- **`dependencies` = what `dist-cli` imports, exactly (P22)**: the eight packages the built binary names — `@duckdb/node-api`, `@jitl/quickjs-wasmfile-release-sync`, `ajv`, `ajv-formats`, `exceljs`, `papaparse`, `quickjs-emscripten-core`, `xlsx`. Everything browser-only moved to `devDependencies`: `@duckdb/duckdb-wasm`, the eight `@codemirror/*`, the three `@fontsource/*`, `@lezer/highlight`, `json-schema-data-dictionary`, and `@jeyabbalas/data-table`. The last of those was reaching the CLI graph for one nine-line function, so `quoteIdentifier` was lifted into `src/core/sql-identifier.ts` and the seven `src/core/**` value imports were repointed at it; the package stays a devDependency so `tests/unit/core/sql-identifier.test.ts` can keep asserting the two copies agree character for character (the browser grids quote the same names). `tests/cli/dependencies.test.ts` parses `dist-cli/*.mjs` with `es-module-lexer` and asserts set equality in **both** directions — an undeclared import is invisible to the build, to `npm pack` and to every test in this repo, and fails only on a stranger's machine with `ERR_MODULE_NOT_FOUND`. It also asserts `optionalDependencies` and `peerDependencies` are absent: npm's `omit` default is `[]`, so both install by default and would otherwise slip past the set-equality check.
...
- **Caveat, documented not fixed (P22): `xlsx` installs from a non-registry host.** SheetJS left npm at 0.18.5; `xlsx@0.20.3` resolves to `https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz`, so `npm i -g quac` fetches one tarball from `cdn.sheetjs.com` rather than `registry.npmjs.org`. It is pinned and integrity-checked (`sha512-oLDq3j…`, Apache-2.0) and `npm ci` verifies that hash, so the risk is availability and policy, not silent substitution: an air-gapped installer, a corporate registry mirror, or an allowlisting proxy will fail the install with a fetch error rather than a resolution error. There is no fix that keeps the capability — the registry's 0.18.5 predates the API this code uses, and vendoring the tarball would put ~800 KB of third-party build output in the repo. Recorded so an installer hitting it can recognize it, and so a future decision to drop xlsx support (or pin to the stale registry copy) is made deliberately.

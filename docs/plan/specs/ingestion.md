...
> Audience: P05 (dataset ingestion & display), P06 (schema slot), P12 (rules slot), P16 (URL loading), P21 (headless CLI intake mirrors §2 formats + §5 guardrails; contract in `headless.md` §8).
...
- status `Badge`: Empty / Loading / Valid / Warning / Error
  — **`Loading` covers the FETCH window itself, not just the work after the bytes land (UX-04).** Both check-source
  stores enter `phase: 'loading'` before their first `await` and hold it unbroken through fetch → parse/crawl → lint,
  and both `summarizeSlot`s test that phase BEFORE any emptiness guard — a first load has no files and a loading
  schema has a null set, so either guard placed first swallows the whole window and, with it, the Clear below. The
  cards mirror it: the URL field reads `Fetching…` while the phase is loading (the schema card derives this from the
  phase rather than latching around the call, so a clear releases it even though a hung request never settles). Every
  path out of a load must land the phase somewhere — including "every URL failed", where nothing downstream runs.
...
- persistent one-line hint on the view (P19b copy): "Files stay in this browser and your session resumes on reload —
  Reset in the header starts fresh." The URL half of the old promise still holds, for **replacements** too (UIX-10):
  every successful load by URL writes itself into the address bar, so fetching a second dataset over a URL-loaded one
  leaves a fragment that reloads the *second* file, not the one the link arrived with. Loading over a URL-loaded slot
  with an *upload* drops that param instead — and since P19b the upload itself resumes from the device-local session
  (§6) rather than dying with the tab.

**Every slot is clearable (UIX-7).** Each card's Clear (and the run bar's `Clear all inputs`) removes that input from the session outright, and the contract is the same on every path: an explicit clear **invalidates the run** (`app/runInvalidation.ts` bumps `store.runEpoch`, cancels an in-flight run, empties the pill/panels — unlike a *replacement* or a Studio edit, which keep the report for consultation), re-syncs the address bar from the REMAINING live sources (a reload cannot resurrect a cleared input; Back cannot un-clear; `config=` always drops with the remainder materialized inline), empties the card's own **typed URL** (UX-06: `createUrlField.clear()`, driven for all three slots from `app/clearInputs.ts` through a registered card hook — a slot that no longer holds a file must stop naming it), and announces with one polite toast. That wipe is bound to the EXPLICIT clears, not to a slot projecting `empty`: the schema store also reaches `phase: 'empty'` when a load throws, and a failed load has to keep what was typed — the field carries the user's typing, not a mirror of the slot (a share-link boot fills all three slots with all three fields empty). The one non-clear that empties a field is an abandoned URL load — the SheetPicker's **Cancel** on the URL leg, where the slot reverts to the previous dataset and the field would otherwise name a workbook that was never loaded; the file leg and every failure path leave it alone. Since UIX-10 the address-bar rewrite is not clear-specific: the clears call the same single writer every *load* goes through (`app/hashSync.ts`, `history.replaceState`, `url-params.md §1/§2`), and keep calling it explicitly so a clear during the boot window — before the store effect is armed — is still covered. A dataset clear additionally drops `quac_raw/typed/work` + the `data` view best-effort through `peekBridge()` (never boots WASM) while holding the card's busy latch, and disposes the report grid; a check-source-only clear strips run paint (annotations, offender filter, cap banner, header tooltips) but keeps the data grid. Confirmation only when unsaved Rule Studio work would be lost; clear-all always confirms. Schema/rules Clear stays enabled during `loading` and doubles as the cancel for a hung fetch (the stores' load tokens discard the late completion) — and being the cancel, it also releases the CARD's own busy latch (UX-04: `run()`'s `.finally()` belongs to a request that may never settle, so without this the URL field stays disabled at `Fetching…` over an `Empty` badge — and the release belongs to the shared clear hook, not the card's own button, or `Clear all inputs` mid-fetch leaves the same dead field, UX-06); only the dataset Clear disables while busy (ingest is uninterruptible mid-CTAS, and `runIngest` would overwrite the cleared slot on settle). Pinned dialog/toast copy in `ui-design.md §5`.
...
## 6. Persistence policy (decided; amended P19b, owner-directed)

**Inputs-only resume, device-local.** The session survives reload in IndexedDB (`quac-session`, `architecture.md §8.5`): the dataset's ORIGINAL bytes (`DatasetSession.source`, replayed through the real ingest with the sheet choice pinned — restore never re-prompts), schema intake entries (upload paths preserved verbatim on replay; the chosen root rides the `index=` share id so the IndexPicker never re-opens for a decided set), rules text serialized through the §7 writer (per-file `sourceUrl` provenance + Studio dirty marks intact), the Studio session (selected file, open drawer, unsaved draft), and the Apply-corrections toggle. **Never the QC report** — a restored session is indistinguishable from a fresh load and the user clicks Run QC again (consent to compute). Restore is **route-independent** (UIX-19): the shell mounts the Load view eagerly on every route, so the dataset replay runs wherever the reload lands (`#/report`, `#/studio`) instead of waiting for a Load-tab visit; the Studio workspace defers a restore that names a rules file the still-loading slot hasn't published yet, applying it on that publish (or on settle, restoring whatever still exists), and re-lints an open drawer's draft when the dataset lint context changes — a restored drawer must not keep a stale "SQL checks are pending" hint beside live data.

Write-through rules (`app/sessionPersistence.ts`): ONE effect over the live stores; slots in a `loading` phase never flush (a hung fetch must not persist a stuck badge — the settle publish re-fires the effect); dataset blob flushes immediately on a generation change, schema/rules/prefs coalesce 500 ms, Studio typing 1 s; `visibilitychange → hidden` flushes pending (debounce loss window ≤1 s on a hard tab-kill); every flush rewrites `meta` from `buildSyncedConfig` — the same writer as the address bar, so bar and stored config cannot disagree. The all-empty state **clears the DB** (empty ≡ absent), which is what keeps "a cleared session stays cleared across reload" true by construction; `Reset`/`Clear all inputs` additionally purge explicitly (timer cancel + presence-hint drop). Multi-tab: one transaction per flush (records never torn), last-flush-wins — documented, not solved.

Fail-open contract: IDB unavailable (private modes), corrupt records, or a foreign snapshot version mean today's boot verbatim — a bad `meta` voids the snapshot and best-effort purges; any other bad record discards itself alone. A failed save costs at most one toast per session; the app never breaks because storage did. Known fidelity losses (accepted): the schema card's "Ignored files" list (non-JSON uploads) does not survive, and original `E_FETCH` findings from a partly-failed URL load do not reproduce. `localStorage` still holds only trivial UI prefs plus the `quac.session.hint` presence flag (anti-flash, `architecture.md §8.5`). The hash fragment remains the only *shareable* persistence (`url-params.md §1`). P22 owns the README privacy rewording.

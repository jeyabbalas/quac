...
4. **SQL dry-run** (needs dataset): per rule, `EXPLAIN` of the EXACT wrapped query the engine will run (row: viol-select wrapper; correct: the rebuild SELECT; dataset: the statement after the single-statement check). DuckDB binder errors surfaced with file + ruleId + rowNumber + CSV column; smart-quote detection adds the word-processor hint. The engine's text is **always** kept verbatim in `detail`; `message` carries it verbatim too, with **one** exception (UX-08) — the **untyped-column class** of V23, where the binder refuses to compare or calculate because a column is stored as text. That class alone gets a plain-language `message` naming the text columns and both remedies (load a JSON Schema, or `TRY_CAST` in the rule), because a `'+(VARCHAR, VARCHAR)'` is not something a data steward can act on and the cause is usually one click they just made. It **fails closed**: recognition needs `Binder Error` + a `VARCHAR` token + one of the four phrase families (`No function matches`, `Could not choose a best candidate`, `Cannot compare values of type`, `Cannot mix values of type`) **and** at least one implicated VARCHAR column, or the engine's own words stand. Matched on the FIRST LINE only — the `LINE 1:` echo can carry a user's own `CAST(x AS VARCHAR)`. The columns come from what each wrapper was built from (a column-scope assertion's text names none) unioned with a word-bounded scan of the rule text (which is what reaches a referenced-but-untargeted column). `TRY_CAST` names a real column only when exactly one is implicated; with several the binder does not say which it choked on.

...
Boot flow (`main.ts`): parse fragment → slots auto-load with progress → statuses land → if complete, "Run QC" is primed but idle. All three legs run on EVERY route (UIX-19): the shell mounts the Load view eagerly, so a `data=` opened on `#/report` ingests at boot instead of parking until the Load tab is visited — and the bar keeps `data=` instead of transiently dropping it. `applyBootConfig` then **arms** the address-bar sync, and only then: the three legs load concurrently, so an effect armed at t0 would fire when the first one resolves and drop the params of the ones still in flight. Every exit path arms, including the empty-config and manifest-failure paths.
